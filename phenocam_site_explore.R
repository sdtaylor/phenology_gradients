...
ggplot() + 
  ...
  geom_point(data=grassland_sites, aes(x=lon, y=lat), size=2.5) + 
  geom_label_repel(data=grassland_sites, aes(x=lon, y=lat,label=site), size=2.5) + 
  theme_bw() +
  #ylim(35,55)+
  #xlim(-100,-65)+
...
# sweetbriargrass: single ROI of grass -Nevermind it's only listed as AG.
# harvardfarmnorth: hmm, 2 ROIS, says they're different but they look the same. use GR_1000
selected_gr_sites = c('marena','konza','uiefprairie','ninemileprairie','oakville','harvardfarmnorth')

# forest sites I chose by hand
# there's a *bunch*, so inital filter is within latitude (35,55), longitude (-100,-65) and has start dates < 2015 and
...
selected_forest_sites = c('shiningrock','harvard','proctor','willowcreek','boundarywaters','shalehillsczo')
